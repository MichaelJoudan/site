...
(() => {
  ...
  const readTheme = () => { try { return localStorage.getItem("ib-theme"); } catch { return null; } };
  const applyTheme = (t) => {
    if (t) document.documentElement.setAttribute("data-theme", t);
    else document.documentElement.removeAttribute("data-theme");
    if (themeBtn) themeBtn.setAttribute("aria-label", "Theme: " + (t || "system"));
  ...
  themeBtn && themeBtn.addEventListener("click", () => {
    ...
    try { next ? localStorage.setItem("ib-theme", next) : localStorage.removeItem("ib-theme"); } catch {}
    applyTheme(next);
  });
  ...
  /* ---------- Routing (hash) ---------- */
  const ROUTES = ["base", "strategies", "signals", "atlas"];
  function routeFromHash() {
    const h = (location.hash || "").replace(/^#\/?/, "").split("?")[0];
    return ROUTES.includes(h) ? h : "base";
  }
  function go(name, { push = true } = {}) {
    const active = routeFromHash();
    if (push && active !== name) location.hash = "#/" + name;
    $$(".route").forEach((r) => {
      const on = r.id === "route-" + name;
      r.classList.toggle("is-active", on);
      r.classList.remove("is-entering");
      if (on && !REDUCED.matches) { void r.offsetWidth; r.classList.add("is-entering"); }
    });
    $$(".seg a").forEach((a) => {
    ...
    document.title = ({
      base: "Intelligence Base", strategies: "Strategies", signals: "Signals", atlas: "World Atlas"
    })[name] + " — " + window.META.owner;
  ...
  /* ---------- Boot ---------- */
  function boot() {
    $$("[data-fill='owner']").forEach((n) => (n.textContent = window.META.owner));
    $$("[data-fill='tagline']").forEach((n) => (n.textContent = window.META.tagline));
    $$("[data-fill='asof']").forEach((n) => (n.textContent = window.META.asof));
    $$("[data-fill='role']").forEach((n) => (n.textContent = window.META.role + " · " + window.META.city));
    $$("[data-fill='disclaimer']").forEach((n) => (n.textContent = window.DISCLAIMER));
    ...
    renderStats(); renderStrategies(); renderEvents();
